...
这一层调 LLM(OpenAI 兼容接口),把小说分块送入,收集 JSON 结果。
...
from .llm_client import LLMClient, split_text_to_chunks, DEFAULT_CHUNK_SIZE
...
class Director:
    ...
    def analyze(self, novel_text: str) -> Tuple[DirectorOutput, Optional[str]]:
        """
        分析整篇小说,返回 (指令列表, 错误信息)。
        ...
        for i, chunk in enumerate(chunks):
            ...
            result, err = self.llm.chat_json(SYSTEM_PROMPT, user_content)
            if err:
                return all_instructions, f"第 {i+1} 块分析失败: {err}"

            # 解析 + 校验
            block_instructions, validate_err = self._parse_and_validate(result, chunk)
            if validate_err:
