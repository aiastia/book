...
10. 质量评分（11维度 + 评分理由，各1-10分，可有一位小数）。
    评分基准：客观评分、保留区分度，不要无脑给高分。评分要有区分度——同一章里强项要敢上 9，弱项要敢压到 7 以下，不要让所有维度挤在 7.8-8.5 的窄带里变成"安慰赛"。每个维度的分数必须和它自己写的理由自洽：理由里指出了缺陷（套路/突兀/堆砌/平实等），该维度的分数就应体现出扣分，不能一边写"修辞亮点不多"一边给 7.8。
    基础维度分档参考（pacing/engagement/coherence/writing_quality/dialogue_quality/world_consistency/plot_logic）：
      9.0-10：该维度有突出亮点或标杆级表现；
      8.0-8.9：表现扎实、无明显问题；
      7.0-7.9：合格但有可指出的具体短板（如句式平实、节奏略散、视角跳切、功能性对白偏多）；
      6.0-6.9：存在明显硬伤（套路化严重、逻辑断裂、信息堆砌、人设崩坏）；
      6.0 以下：严重失败。
    注意：本章一旦出现以下情形，对应维度必须扣分、不得塞进 8 分档：
      - 修辞套路化/比喻陈腐（"像闪电""唰地红了""像根线"等）→ writing_quality 压到 7.5 以下；
      - 短篇幅内频繁切换视角或场景（≥4 次）且衔接生硬 → coherence 压到 8.0 以下、pacing 同步扣；
      - 一次性堆砌大量设定/术语（转世+龙族+暗黑神官+胎记同时抛出而无消化空间）→ engagement 与 attraction 各扣 0.5-1；
      - 对话仅承担信息传递、缺乏角色个性 → dialogue_quality 压到 7.5 以下。
    开篇章（第1章或前几章）特判：coherence 没有"前文"可衔接，只评"本章内部逻辑是否自洽、场景衔接是否顺畅"，不要因"与前文衔接好"而虚高。
    - overall: 整体质量（综合本章的写作水准，取各维度加权后的总体印象，不要简单取平均）
    - pacing: 节奏把控（事件密度、推进速度是否合适，视角/场景切换是否过频）
    - engagement: 吸引力（整章阅读体验：读者读完后是否想继续读下一章，综合考量节奏、信息密度、情感投入。注意：它评的是"全章阅读体验"，与下方 attraction 评"开头能否瞬间抓住新读者"职责不同，二者可以相差 0.5-1 分但必须有各自独立的依据，不得互相抄写）
    - coherence: 连贯性（场景衔接是否顺畅、逻辑是否自洽；开篇章只评内部自洽）
    - writing_quality: 文笔质量（语言表达、修辞是否新鲜而非套路、画面感）
    - character_depth: 角色塑造（人物是否立体、动机是否合理、行为是否符合人设）
    - dialogue_quality: 对话质量（对话是否自然、是否有信息量、是否体现角色个性）
    - world_consistency: 世界观一致性（设定是否与前文矛盾、力量体系是否自洽；开篇章评设定本身是否清晰自洽）
    - plot_logic: 剧情逻辑（事件因果链是否合理、转折是否有铺垫）
    - attraction: 番茄吸量力（开头/标题/设定是否足够抓人，能否让新读者一眼入坑）。
        标尺：开头有强感官钩子或悬念 8 分起；同时具备独特设定/反差感 8.5-9。但若开头几段同时抛出 ≥3 个设定概念（如转世+龙族+暗黑神官+胎记）且无消化空间，属于信息堆砌，必须从基准分扣 0.5-1，不得仍给 8.8/9。即"钩子多"不等于"吸量力高"，堆砌反而劝退新读者。
    ...
    - score_justification: 评分理由（必填，中文）。
        ...
        【分数与理由必须自洽】每一行的分数必须和该行文字描述的缺陷/亮点一致。禁止出现"理由里指出了套路/突兀/堆砌/平实等短板，分数却仍给 8.0+"的矛盾。同一章若多个基础维度理由都带"略/稍"类短板，这些维度的分数必须真正落到 7.x 档，不得全部挤在 8 分以上。
        示例：
