...
async def _current_project(db: AsyncSession, user: User) -> Optional[Project]:
...
@router.get("/me")
async def compat_me(user: Optional[User] = Depends(get_user_dev)):
    if not user:
...
# ---------- 仪表盘 ----------
@router.get("/stats")
async def stats(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    ...
    pids = [p for p in (await db.execute(select(Project.id).where(Project.user_id == user.id))).scalars().all()]
    chapters = words = characters = 0
    if pids:
        chapters = (await db.execute(select(func.count(Chapter.id)).where(Chapter.project_id.in_(pids)))).scalar() or 0
        words = (await db.execute(select(func.coalesce(func.sum(Chapter.word_count), 0)).where(Chapter.project_id.in_(pids)))).scalar() or 0
        characters = (await db.execute(select(func.count(Character.id)).where(Character.project_id.in_(pids)))).scalar() or 0
...
async def recent_edits(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    return await _recent_activities(db, user, 5)


@router.get("/recent-activity")
async def recent_activity(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    return await _recent_activities(db, user, 5)


# ---------- 书架（= 项目列表） ----------
@router.get("/books")
async def books(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """前端"我的书架" → 用户所有项目。"""
    if not user:
        return []
    projects = (
    ...
    for p in projects:
        ...
        words = (await db.execute(
            select(func.coalesce(func.sum(Chapter.word_count), 0)).where(Chapter.project_id == p.id)
        )).scalar() or 0
        items.append({
            "id": p.id,
            "title": p.title,
            ...
            "chapters": ch_count,
            "words": f"{words:,}",
            "updated": (p.updated_at or p.created_at).strftime("%Y-%m-%d") if (p.updated_at or p.created_at) else "",
            "tag": p.genre or "其他",
...
@router.post("/books")
async def create_book(req: BookCreate, db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """新建书（项目）。"""
    if not user:
...
# ---------- 当前项目下的章节/大纲/角色/世界观 ----------
@router.get("/chapters")
async def chapters(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """前端"故事章节"页 → 当前项目的章节。"""
    ...
    rows = (
        await db.execute(
            select(Chapter).where(Chapter.project_id == proj.id).order_by(Chapter.chapter_number)
        )
    ).scalars().all()
    now = datetime.utcnow()
    items = []
...
@router.get("/outline")
async def outline(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """前端"故事大纲"页 → 当前项目的大纲。"""
    proj = await _current_project(db, user)
    if not proj:
        return []
    rows = (
        await db.execute(
            select(Outline).where(Outline.project_id == proj.id).order_by(Outline.chapter_number)
        )
    ).scalars().all()
    return [{
        "id": o.id,
...
async def characters(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """前端"角色设定"页 → 当前项目的角色。"""
    proj = await _current_project(db, user)
    if not proj:
        return []
    rows = (
        await db.execute(select(Character).where(Character.project_id == proj.id))
    ).scalars().all()
    items = []
    for c in rows:
...
async def worldview(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
...
# ---------- 提示词 ----------
@router.get("/prompts")
async def prompts(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
    """提示词模板列表。"""
    base_q = select(PromptTemplate)
    if user:
        rows = (await db.execute(
            base_q.where((PromptTemplate.user_id == user.id) | (PromptTemplate.user_id.is_(None)))
        )).scalars().all()
    else:
        rows = (await db.execute(
            base_q.where(PromptTemplate.user_id.is_(None))
        )).scalars().all()
    return [{
        "title": p.name,
...
async def writing_log(db: AsyncSession = Depends(get_db), user: Optional[User] = Depends(get_user_dev)):
