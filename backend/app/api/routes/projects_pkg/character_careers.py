...
async def list_char_careers(
    ...
    if career_ids:
        careers = (
            (await db.execute(select(Career).where(Career.id.in_(career_ids)))).scalars().all()
        )
        for c in careers:
            stages = c.stages or []
            career_map[c.id] = {
    ...
    return [
        {
            **r.to_dict(),
            ...
            "stage_name": (
                career_map.get(r.career_id, {}).get("stages", [])[r.current_stage - 1]
...
async def create_char_career(
    ...
    # 同步角色冗余字段
    char = (
        await db.execute(select(Character).where(Character.id == req.character_id))
    ).scalar_one_or_none()
    if char:
        if req.career_type == "main":
            ...
            career = (
                await db.execute(select(Career).where(Career.id == req.career_id))
            ).scalar_one_or_none()
            subs.append(
                {
                    "career_id": req.career_id,
                    "name": career.name if career else "",
...
async def update_char_career(
    ...
    cc = (
        await db.execute(
            select(CharacterCareer).where(
                CharacterCareer.id == cc_id, CharacterCareer.project_id == project_id
            )
        )
    ).scalar_one_or_none()
    if not cc:
    ...
    if cc.career_type == "main":
        char = (
            await db.execute(select(Character).where(Character.id == cc.character_id))
        ).scalar_one_or_none()
        if char:
...
async def auto_assign_careers(
    ...
    career_list = "\n".join(
        f"- ID:{c.id} {c.name}（{c.career_type}，{c.category or '通用'}）" for c in careers
    )
    # 已有主职业的角色
    assigned_ids = [
        cc.character_id
        for cc in (
            await db.execute(
                select(CharacterCareer).where(
                    CharacterCareer.project_id == project_id, CharacterCareer.career_type == "main"
                )
            )
        )
        .scalars()
    ...
    char_list = "\n".join(
        f"- ID:{c.id} {c.name}（{c.role or '角色'}，{c.main_career_stage_desc or '无职业'}，性格：{(c.personality or '未知')[:50]}）"
        for c in chars
    )
