...
async def validate_outline_entities(db, project_id: int, title: str, genre: str, world_info: str, ai_client) -> int:
    """验证大纲中涉及的角色和组织是否都已创建，缺失则自动补充。返回补全数量。"""
...
要求：每个角色包含 name、role（配角）、gender、age、identity、personality（80-150字）、background（80-150字）、occupation、story_goal、motivation、ability。返回纯JSON数组。"""}], temperature=0.8, max_tokens=4096)
...
async def generate_outlines(project_id: int, req: OutlineGenerateRequest, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    # 验证大纲中涉及的角色/组织是否都已创建
    world_ctx = _build_world_context_for_validate(project)
    asyncio.create_task(validate_outline_entities(db, project_id, project.title, project.genre or "网文", world_ctx, ai_client))

    return {"outlines": created, "count": len(created)}


def _build_world_context_for_validate(project) -> str:
    parts = []
