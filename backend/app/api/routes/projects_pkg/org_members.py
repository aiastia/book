...
async def update_org_tree(
    ...
    if "parent_org_id" in req:
        ...
        if new_parent:
            parent = (
                await db.execute(select(Organization).where(Organization.id == new_parent))
            ).scalar_one_or_none()
...
async def _generate_members_for_org(
    db: AsyncSession, project_id: int, org_id: int, user_prompt: str = "", user_id: int = None
) -> list[dict]:
...
async def generate_all_members(
    project_id: int,
    req: OrgMembersGenerateReq = None,
    db: AsyncSession = Depends(get_db),
    user=Depends(get_current_user),
):
