...
async def _resolve_continuous_chapters(
    db: AsyncSession, project_id: int, start: int, count: int
) -> list[int]:
...
async def batch_generate(
    ...
    if req.start_chapter_number and req.count:
        ...
        chapter_ids = await _resolve_continuous_chapters(
            db, project_id, req.start_chapter_number, req.count
        )
        await db.commit()
        ...
        raise HTTPException(
            400, "请指定 start_chapter_number+count（连续模式）或 chapter_ids（手动模式）"
        )
