...
async def list_memories(
    ...
    q = q.order_by(StoryMemory.chapter_number.desc(), StoryMemory.importance.desc()).limit(min(limit, 200))
    memories = (await db.execute(q)).scalars().all()
    return [_to_dict(m) for m in memories]


@router.get("/{project_id}/memories/stats")
async def memory_stats(project_id: int, db: AsyncSession = Depends(get_db), user=Depends(get_current_user)):
    ...
    total = await db.scalar(select(func.count(StoryMemory.id)).where(StoryMemory.project_id == project_id))
    ...
    return {"total": total or 0, "by_type": by_type, "vectorized": vectorized or 0, "embedding": embedding_status}
...
async def create_memory(
    project_id: int, req: MemoryCreateReq,
    db: AsyncSession = Depends(get_db), user=Depends(get_current_user),
):
    """手动添加记忆（自动写入向量库）"""
    await get_user_project(db, project_id, user)
    ...
    try:
        ...
        vid = await vs.add_memory(
            user_id=user.id, project_id=project_id, memory_id=m.id,
...
async def update_memory(
    project_id: int, memory_id: int, req: dict,
    db: AsyncSession = Depends(get_db), user=Depends(get_current_user),
):
    m = (await db.execute(
        select(StoryMemory).where(StoryMemory.id == memory_id, StoryMemory.project_id == project_id)
    )).scalar_one_or_none()
    if not m:
...
async def clear_memories(
    project_id: int, memory_type: str = None,
    db: AsyncSession = Depends(get_db), user=Depends(get_current_user),
):
    """清空记忆（可按类型）。同步清理向量库。"""
    await get_user_project(db, project_id, user)
...
async def semantic_search(
    project_id: int, req: SemanticSearchReq,
    db: AsyncSession = Depends(get_db), user=Depends(get_current_user),
):
