...
def get_token(args):
    r = requests.post(
        f"{args.api}/api/auth/login", json={"username": args.user, "password": args.password}
    )
    r.raise_for_status()
...
def cmd_register(args):
    r = requests.post(
        f"{args.api}/api/auth/register",
        json={"username": args.user, "password": args.password, "email": args.email},
    )
    if r.ok:
        print(f"✓ 注册成功: {args.user}")
    else:
        print(f"✗ {r.json().get('detail', '注册失败')}")
        sys.exit(1)
...
def cmd_projects(args):
    ...
    for p in r.json():
        print(
            f"  [{p['id']}] {p['title']} ({p['genre']}) - {p['chapter_count']}章 {p['current_word_count']}字"
        )
...
def cmd_create(args):
    ...
    if r.ok:
        print(f"✓ 项目创建成功: #{r.json()['id']}")
    else:
        print(f"✗ {r.json().get('detail', '创建失败')}")
        sys.exit(1)
...
def cmd_gen_outline(args):
    ...
    r = requests.post(
        f"{args.api}/api/projects/{args.project}/outlines/generate",
        headers=headers(token),
        json=data,
    )
    if r.ok:
        ...
        print(f"✗ {r.json().get('detail', '生成失败')}")
        sys.exit(1)
...
def cmd_chapters(args):
    ...
    for c in r.json():
        ...
        print(
            f"  {status} [{c['id']}] 第{c['chapter_number']}章 {c['title']} ({c['word_count']}字)"
        )
...
def cmd_gen_chapter(args):
    token = get_token(args)
    r = requests.post(
        f"{args.api}/api/projects/{args.project}/chapters/{args.chapter}/generate",
        headers=headers(token),
    )
    if r.ok:
        ch = r.json()
...
def cmd_foreshadows(args):
    ...
    for f in r.json():
        print(
            f"  [{f['status']}] {f['title']} (埋:{f['plant_chapter_number'] or '-'} 回:{f['target_resolve_chapter_number'] or '-'})"
        )
...
def cmd_plan_foreshadows(args):
    token = get_token(args)
    r = requests.post(
        f"{args.api}/api/projects/{args.project}/foreshadows/plan", headers=headers(token)
    )
    if r.ok:
        fs = r.json()
        ...
        for f in fs:
            print(
                f"  {f['title']} → 埋:{f['plant_chapter_number']} 回:{f['target_resolve_chapter_number']}"
            )
    else:
        print(f"✗ {r.json().get('detail', '规划失败')}")
        sys.exit(1)
...
def cmd_add_model(args):
    ...
    if r.ok:
        print(f"✓ 模型添加成功: {r.json()['name']}")
    else:
        print(f"✗ {r.json().get('detail', '添加失败')}")
        sys.exit(1)
...
def main():
    ...
    sub.add_parser("register", help="注册用户").add_argument("user").add_argument(
        "password"
    ).add_argument("--email")
    sub.add_parser("projects", help="列出项目")
    cp = sub.add_parser("create", help="创建项目")
    cp.add_argument("title")
    cp.add_argument("--genre", default="玄幻")

    sub.add_parser("outlines", help="列出大纲").add_argument("project", type=int)
    gop = sub.add_parser("gen-outline", help="AI 生成大纲")
    gop.add_argument("project", type=int)
    gop.add_argument("--synopsis", required=True)
    gop.add_argument("--count", type=int, default=10)

    sub.add_parser("chapters", help="列出章节").add_argument("project", type=int)
    gcp = sub.add_parser("gen-chapter", help="AI 生成章节")
    gcp.add_argument("project", type=int)
    gcp.add_argument("chapter", type=int)
    gcp.add_argument("--output", "-o")
    ...
    mp.add_argument("--provider", required=True)
    mp.add_argument("--base-url", required=True)
    ...
    cmds = {
        ...
        "gen-outline": cmd_gen_outline,
        "chapters": cmd_chapters,
        "gen-chapter": cmd_gen_chapter,
        "foreshadows": cmd_foreshadows,
        "plan-foreshadows": cmd_plan_foreshadows,
