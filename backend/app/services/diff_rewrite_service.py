...
async def _call_rewrite_api(
    ...
    system_prompt = (
        ...
        "如果原句已经够好，原样返回。\n"
        "特殊处理1：「不是A，是B」句式改成直接陈述，彻底去掉「不是…是…」结构。\n"
        "  例：「不是矿道，是裂隙」→「矿道裂成了一条向下的裂隙」\n"
        "  例：「不是消失，是变形」→「轮廓正在变形」\n"
        "  例：「不是沉默，是在等」→「他不说话，但没走，像是在等什么」\n"
        "  例：「不是刀，是骨头」→「那东西不是铁打的，是骨头磨的」\n"
        "  ⚠️ 改写后绝对不能再出现「不是」「是个」这两个词连用。把 A 和 B 融进一句全新的话，不要保留否定结构。\n"
        "  ⚠️ 带破折号的变体也要改：「不是后来那个摄政王。是个——法师」→「那时他还不是摄政王，只是个法师」\n"
...
async def diff_rewrite(
    ...
    # 替换后可能产生连续标点（AI 输出末尾句号与原文句号叠加），统一清理
    import re as _re
    text = _re.sub(r"[，。！？]{2,}", lambda m: m.group()[-1], text)
