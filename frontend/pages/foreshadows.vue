...
import type { Character, Foreshadow } from '~/composables/api/types'
useHead({ title: '伏笔管理 — 墨语' })
const { currentProjectId } = useProject()
if (!currentProjectId.value) await navigateTo('/books')
const msg = useMessage()
const { data: foreshadows, refresh: refresh } = await useFetch<Foreshadow[]>(() => `${useRuntimeConfig().public.apiBase}/api/projects/${currentProjectId.value}/foreshadows`)
