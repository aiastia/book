...
async function onSubmit() {
  ...
  try {
    ...
    error.value = formatError(e, '注册失败，请稍后再试')
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="login-wrap">
    <div class="login-card">
      ...
      <p class="login-subtitle">注册 墨语 开始你的创作之旅</p>
      ...
      <div class="login-form">
        ...
        <div class="form-item">
          ...
          <input v-model="form.nickname" type="text" class="form-input" placeholder="显示名称，不填则用用户名" />
        </div>
        <div class="form-item">
          <label class="form-label">密码</label>
          <input v-model="form.password" type="password" class="form-input" placeholder="至少 6 个字符" />
        </div>
        <div class="form-item">
          <label class="form-label">确认密码</label>
          <input v-model="form.confirmPassword" type="password" class="form-input" placeholder="再次输入密码" @keyup.enter="onSubmit" />
        </div>
      ...
      <p class="login-foot">
        已有账号？<NuxtLink to="/login" class="login-link">去登录</NuxtLink>
      </p>
    </div>
  </div>
</template>
