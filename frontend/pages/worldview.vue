...
const { data: worlds, refresh: refreshWorlds } = await useFetch<WorldSetting[]>(() => `${useRuntimeConfig().public.apiBase}/api/projects/${currentProjectId.value}/worldview`)
const { data: worldCore, refresh: refreshCore } = await useFetch<WorldCore>(() => `${useRuntimeConfig().public.apiBase}/api/projects/${currentProjectId.value}/worldview/core`)
