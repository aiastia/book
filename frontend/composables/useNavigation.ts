...
const projectNav: NavItem[] = [
  { label: '返回书架', to: '/books', icon: 'book' },
  { label: '仪表盘', to: '/dashboard', icon: 'home' },
...
export function useNavigation() {
  ...
  const nav = computed<NavItem[]>(() => {
    ...
    const items = isProject.value ? projectNav : globalNav
    // 项目页面的链接自动追加 ?pid=，刷新时可恢复项目上下文
    if (isProject.value && currentProjectId.value) {
      return items.map(item => ({
        ...item,
        to: item.to + '?pid=' + currentProjectId.value,
      }))
    }
    return items
  })
  ...
  return { nav, isProject, currentPath, adminNav, showAdminEntry }
}